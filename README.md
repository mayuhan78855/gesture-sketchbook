...
用摄像头手势驱动的实时交互粒子 Web 应用，三种视觉模式：**3D 星环**（Three.js，默认）、**粒子**、**笔迹**。张开手掌挥动，星环随你加速旋转；捏合，万千粒子聚合成星球。深空底色 + 青色 HUD，科技感拉满。

![演示动图](assets/demo.gif)

3D 星环（手势控制视角与聚合）与摄像头异常处理：

![3D 星环](assets/screenshots/demo-space-ring.png)
...
默认是 **3D 星环模式**（Three.js），侧栏可切换**粒子模式**与**笔迹模式**：

| 手势 | 动作 | 3D 星环模式 | 粒子模式 | 笔迹模式 |
|------|------|------------|---------|---------|
| 🖐 张开手掌 | 面向镜头移动 | 视角随手掌移动，挥动加速旋转 | 指尖拖出发光粒子流 | 用食指尖绘制霓虹笔迹 |
| ✊ 握拳 | 保持 3 帧 | 星环收缩加速 | 从掌心爆发一圈粒子 | 切换画笔颜色 |
| ☝ 食指上指 | 只伸食指 | 切换俯瞰视角 | 指尖向上喷射离子喷泉 | 打开/关闭构造线 |
| ✌ 剪刀手 | 伸出食指+中指 | 星环分裂成内外双环 | 双指间生成量子涡流 | 切换笔刷粗细 |
| 👍 点赞 | 竖起拇指 | 超新星爆开重组 | 超新星爆发并清空粒子 | 清空画布 |
| 🤏 捏合 | 拇指食指捏在一起 | 万千粒子聚合成星球 | 聚合成一朵缓缓绽放的花，松开消散 | 撤销上一笔 |
...
- **Three.js**——3D 星环模式的 WebGL 渲染（CDN 按需懒加载）
