...
用摄像头手势驱动霓虹粒子的 Web 应用（默认粒子模式，可一键切回笔迹模式）。张开手掌 = 指尖喷出粒子流，握拳 = 能量爆发，捏合 = 指尖聚合一朵缓缓绽放的花、松开即消散——不需要任何外设，一个浏览器就够。深空底色 + 青色辉光 + HUD 取景框，科技感拉满。
...
默认是**粒子模式**（手势驱动粒子效果）；点侧栏「笔迹模式」或地址加 `?mode=ink` 可切换回画笔：

| 手势 | 动作 | 粒子模式效果 | 笔迹模式效果 |
|------|------|-------------|-------------|
| 🖐 张开手掌 | 手掌面向镜头移动 | 指尖拖出发光粒子流 | 用食指尖绘制霓虹笔迹 |
| ✊ 握拳 | 保持 3 帧 | 从掌心爆发一圈粒子 | 切换画笔颜色 |
| ☝ 食指上指 | 只伸食指 | 指尖向上喷射离子喷泉 | 打开/关闭构造线 |
| ✌ 剪刀手 | 伸出食指+中指 | 双指间生成量子涡流 | 切换笔刷粗细 |
| 👍 点赞 | 竖起拇指 | 超新星爆发并清空粒子 | 清空画布 |
| 🤏 捏合 | 拇指食指捏在一起 | 聚合成一朵缓缓绽放的花，松开消散 | 撤销上一笔 |

除了手势，**动作本身也有效**：快速挥动手掌会带起周围的粒子（掌风气流）。
